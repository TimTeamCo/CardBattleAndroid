...
namespace TTAuth
{
    ...
    {
        private Dictionary<Auth.IIdentityType, SubIdentity> m_subIdentities = new Dictionary<Auth.IIdentityType, SubIdentity>();

        public Identity(Action callbackOnAuthLogin)
        {
            m_subIdentities.Add(Auth.IIdentityType.Local, new SubIdentity());
            m_subIdentities.Add(Auth.IIdentityType.Auth, new SubIdentity_Authentication(callbackOnAuthLogin));
        }
