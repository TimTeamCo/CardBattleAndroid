...
namespace TTRelay
{
   ...
   /// If you are using the Unity Networking Package, you can use their Relay instead of building your own packets.
    ...
    public class RelayUtpHost : RelayUtpClient, IReceiveMessages
    {
        ...
        {
            WriteByte(m_networkDriver, conn, m_localUser.ID, UtpMessageType.PlayerApprovalState, (byte)result);
        ...
        protected override bool CanProcessDataEventFor(NetworkConnection conn, UtpMessageType type, string id)
        {
            ...
            return id != m_localUser.ID && (m_localLobby.LobbyUsers.ContainsKey(id) && m_connections.Contains(conn) || type == UtpMessageType.NewPlayer);
        }

        protected override void ProcessNetworkEventDataAdditional(NetworkConnection conn, UtpMessageType msgType, string id)
        {
            // Forward messages from clients to other clients.
            if (msgType == UtpMessageType.PlayerName)
            {
            ...
            else if (msgType == UtpMessageType.Emote || msgType == UtpMessageType.ReadyState)
            {
                byte value = msgType == UtpMessageType.Emote ? (byte)m_localLobby.LobbyUsers[id].Emote : (byte)m_localLobby.LobbyUsers[id].UserStatus;
                foreach (NetworkConnection otherConn in m_connections)
                {
                    if (otherConn == conn)
                        continue;
            ...
            else if (msgType == UtpMessageType.NewPlayer)
            {
                OnNewConnection(conn, id);
            }
            else if (msgType == UtpMessageType.PlayerDisconnect) // Clients message the host when they intend to disconnect, or else the host ends up keeping the connection open.
            {
            ...
            if (msgType == UtpMessageType.ReadyState)
        ...
        public void OnReceiveMessage(MessageType type, object msg)
        {
            ...
            else if (type == MessageType.EndGame) // This assumes that only the host will have the End Game button available; otherwise, clients need to be able to send this message, too.
            {
                foreach (NetworkConnection connection in m_connections)
                    WriteByte(m_networkDriver, connection, m_localUser.ID, UtpMessageType.EndInGame, 0);
            }
        }
        ...
        private void CheckIfAllUsersReady()
        {
            ...
            if (haveAllReadied && m_localLobby.State == LobbyState.Lobby) // Need to notify both this client and all others that all players have readied.
            {
                ...
                foreach (NetworkConnection connection in m_connections)
                    WriteByte(m_networkDriver, connection, m_localUser.ID, UtpMessageType.StartCountdown, 0);
            }
            else if (!haveAllReadied && m_localLobby.State == LobbyState.CountDown) // Someone cancelled during the countdown, so abort the countdown.
            {
                ...
                foreach (NetworkConnection connection in m_connections)
                    WriteByte(m_networkDriver, connection, m_localUser.ID, UtpMessageType.CancelCountdown, 0);
            }
        }
        ...
        public void SendInGameState()
        {
            ...
            foreach (NetworkConnection connection in m_connections)
                WriteByte(m_networkDriver, connection, m_localUser.ID, UtpMessageType.ConfirmInGame, 0);
        }
